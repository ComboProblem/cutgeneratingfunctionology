...
def finite_group_order_from_function_f_oversampling_order(fn, f=None, oversampling=None, order=None):
    ...
    if is_rational_bkpt_f:
        ...
        if oversampling is None:
            compendium_procedures_logger.info("Rational breakpoints and f; using group generated by them, (1/%s)Z" % q)
            return q
        else:
            grid_nb = oversampling * q
            compendium_procedures_logger.info("Rational breakpoints and f; using group generated by them, refined by the provided oversampling factor %s, (1/%s)Z" % (oversampling, grid_nb))
            if oversampling >= 3 and fn.is_continuous_defined():
                # TODO: Detect and handle the case that fn already was a finite group problem
                compendium_procedures_logger.info("This is a continuous function; because oversampling factor is >= 3, the extremality test on the restricted function will be equivalent to the extremality test on the original function.")
            return grid_nb
    else:
